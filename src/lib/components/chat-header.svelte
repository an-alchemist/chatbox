<script lang="ts">
	...
	import CollectionSelector from './collection-selector.svelte';
	...
	let {
		...
		chat: Chat | undefined;
		readonly: boolean;
		collections?: string[]; // Add this type definition
	} = $props();
...
<header class="bg-background sticky top-0 flex items-center gap-2 p-2">
	...
	{#if !readonly}
		...
		<CollectionSelector {collections} class="order-1 ml-2 md:order-2" />
	{/if}
